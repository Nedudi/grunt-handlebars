...
module.exports = function (grunt) {
  ...
  grunt.registerMultiTask('handlebars_json2template', 'The best Grunt plugin ever.', function () {
    ...
    var config = this.data, arrTemplates = [], langTemplates = [], lang = [], currentBuildVersion = this.data.version;
      ...
      arrTemplates.forEach(function (tmpl) {
        ...
        langTemplates.forEach(function (currTempLang) {
          if (currTempLang.nameTemplate === tmpl.nameTmp && currTempLang['src'][lng]) {
            ...
            data.CURRENT_BUILD_VERSION = currentBuildVersion;
            var compiled = template(data);
            if (tmpl.nameTmp === "index") {
              grunt.file.write(config.path + lng + "/" + nameFile, compiled);
            } else {
              grunt.file.write(config.path + lng + "/" + tmpl.nameTmp + "/" + nameFile, compiled);
            }
          }
        });
      });
